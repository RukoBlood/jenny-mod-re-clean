...
public class GalathModel extends GirlModel<GirlEntity> {
    ...
    public ResourceLocation getModelLocation(GirlEntity girl) {
        ...
        if (((IGalath) girl).isHuggingManglelie()) {
        ...
        if (Action.isAnyAction(girl, Action.PUSSY_LICKING)) {
            if (girl instanceof GalathEntity) {
                if (!this.mc.isGamePaused()) {
                    ...
                    Vector3fSexmodSpecial offset = this.calculatePussyLickingRotationOffset((GalathEntity) girl, partialTicks);
    ...
    Vector3fSexmodSpecial calculatePussyLickingRotationOffset(GalathEntity galath, float renderTicks) {
        return RotationHelper.LerpVector3f(this.getPussyLickingWaveAngles(renderTicks), Vector3fSexmodSpecial.ZERO, galath.getSwordAttackProgres(this.mc.getRenderPartialTicks()));
    }
    ...
    void updateMasturbationHeadLook(GirlEntity girl) {
        if (!ClientProxy.IS_PRELOADING) {
            if (girl.getCurrentAction() == Action.MASTERBATE) {
                EntityPlayer master = girl.getMasterPlayer();
                if (master == null) {
                    master = this.mc.player;
                }
                MolangParser parser = GeckoLibCache.getInstance().parser;
                Vec3d targetOffset = EntityLookVectorHelper.getLookVectorTo(girl, master, this.mc.getRenderPartialTicks()).add(girl.getCachedBoneOffset("head"));
    ...
    void updateSwordAttackBodyOffset(GirlEntity girl) {
        if (girl instanceof GalathEntity) {
            ...
            if (galath.getCurrentAction() != Action.ATTACK_SWORD) {
                ...
                int attackProgress = galath.getSwordAttackProgress();
                ...
                if (ThreadNames.isValueInBounds(attackProgress, 24.0D, 32.0D)) {
                    IBone bodyBone = this.getAnimationProcessor().getBone("body");
                    ...
                    movementOffset = RotationHelper.LerpVec3d(movementOffset, Vec3d.ZERO, progress);
