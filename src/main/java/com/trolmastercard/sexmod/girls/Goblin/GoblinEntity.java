...
implements ai_class30 {
    ...
    public void setDead() {
        ...
        ItemStack itemStack = this.entityDataManager.get(a0);
    ...
    public void void_a(UUID uUID) {
        ...
        this.entityDataManager.set(Q, uUID.toString());
    }

    @Override
    @Nullable
    public UUID java_util_UUID_e() {
        String string = this.entityDataManager.get(Q);
        if ("".equals(string)) {
            return null;
        }
        try {
            return UUID.fromString(this.entityDataManager.get(Q));
    ...
    void void_B() {
        ...
        e3_class2192.entityDataManager.set(a0, itemStack);
    ...
    @Override
    public void void_a(Action action) {
    ...
    public boolean shouldRenderNameTag() {
        ...
        if (!this.entityDataManager.get(aC).booleanValue() && !Minecraft.getMinecraft().player.canEntityBeSeen(this)) {
            return false;
        }
        ...
        block5 : switch (event.getController().getName()) {
            ...
            case "action": {
                ...
                switch (this.currentAction()) {
                    ...
                    case CATCH: {
                        this.createAnimation(String.format("animation.goblin.catch_%sperson", string), true, event);
                        break block5;
                    }
                    case CATCH_BJ: {
                        this.createAnimation(String.format("animation.goblin.catch_%spersonBj", string), true, event);
                        break block5;
                    }
                    case CATCH_BJ_IDLE: {
                        this.createAnimation(String.format("animation.goblin.catch_%spersonBj_idle", string), true, event);
                        break block5;
                    }
                    case START_THROWING: {
